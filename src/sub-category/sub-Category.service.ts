...
export class SubCategoryService {
  ...
  async create(createSubCategoryDto: CreateSubCategoryDto): Promise<ReadSubCategoryDto> {
    ...
    if (!category) {
      throw new HttpException(`Category with ID ${createSubCategoryDto.fkCategory} not found`, HttpStatus.NOT_FOUND);
    }
    ...
    return SubCategoryMapper.entityToReadSubCategoryDto(await this.subCategoryRepository.save(newEntity));
  }
  ...
  async findOne(validId: ValidID): Promise<ReadSubCategoryDto> {
    ...
    if (!entity) {
      throw new HttpException(`SubCategory with ID ${validId.id} not found`, HttpStatus.NOT_FOUND);
    }
  ...
  async findAll(): Promise<ReadSubCategoryDto[]> {
    return this.subCategoryRepository.find({ relations: ['category'] }).then((subs) =>
      subs.map((subCate) => SubCategoryMapper.entityToReadSubCategoryDto(subCate)),
    );
  }
  ...
  async remove(validID: ValidID): Promise<{ message: string; status: HttpStatus }> {
    ...
    const responseDeleted = await this.subCategoryRepository.remove(found);
  ...
  async update(updateSubCategoryDto: UpdateSubCategoryDto): Promise<{ message: string; status: HttpStatus }> {
    ...
    if (!entity) {
      throw new HttpException(`SubCategory with ID ${updateSubCategoryDto.pkSubCategory} not found`, HttpStatus.NOT_FOUND);
    }
    ...
    if (!category) {
      throw new HttpException(`Category with ID ${updateSubCategoryDto.fkCategory} not found`, HttpStatus.NOT_FOUND);
    }
